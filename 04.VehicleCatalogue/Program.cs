...
while (command != "end")
{
    ...
    if (type == "Car")
    {
        Car currentCar = new Car { Brand = brand, Model = model, HorsePower = int.Parse(data[3]) };
        catalog.Cars.Add(currentCar);
    }
    else if (type == "Truck")
    {
        Truck currentTruck = new Truck {  Brand = brand, Model = model, Weight = int.Parse(data[3]) };
...
{
    List<Truck> orederedTrucks = catalog.Trucks.OrderBy(truck => truck.Brand).ToList();
